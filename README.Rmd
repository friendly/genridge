...
In this context,
high multiple correlations among the predictors lead to well-known problems of colinearity
...
* there is also some _structural colinearity_ among the variables `GNP`, `Year`, `GNP.deflator`, `Population`, e.g., `GNP.deflator` is a multiplicative factor to account for inflation.
...
The effects of colinearity can be measured by a variance inflation factor (VIF), the ratio
...
`norm.beta` is a measure of shrinkage, and `det` is a measure of variance.
...
These are the
directions that contribute most to colinearity, for which other visualization methods have
...
_The American Statistician_, **63**, 56–65.
